...
6. 사용자가 선택한 language 값의 언어로 답한다. language가 없으면 한국어로 답한다.
7. 출력은 JSON만 반환한다.
`;
...
exports.handler = async function(event) {
  ...
  const selectedLanguage = payload.language || "Korean";
  const userContent = `${schemaInstruction}\n반드시 ${selectedLanguage}로 답하세요. JSON 키는 유지하고, JSON 값만 ${selectedLanguage}로 작성하세요.\n입력값:\n${JSON.stringify(payload, null, 2)}`;
